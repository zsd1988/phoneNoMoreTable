...
public class PhoneGetMessageThread extends Thread {
    private static Logger logger = Logger.getLogger(PhoneGetMessageThread.class);

    private PhoneSocket socket;
    public boolean isRun = true;
    public InputStream inputStream;

    public PhoneGetMessageThread(PhoneSocket socket) {
        this.socket = socket;
    }
...
//                            logger.info("收到数据长度：" + data.length());
                    ...
                    logger.info(socket.phone + " 解析数据失败：" + e.getMessage());
                    result = new byte[0];
