...
Class.extend = function(members) {
	...
	// build the systemManager prototype for the subclass, which is an new object chained to the superclass's prototype
...
var Popover = Class.extend({
	...
	// Triggered when the userSystem clicks *anywhere* in the document, for the autoHide feature
...
var DragListener = fc.DragListener = Class.extend({
	...
	// Call this when the userSystem does a mousedown. Will probably lead to startListening
	...
	// Called when the userSystem moves the mouse
	...
	// Called when the userSystem does a mouseup
	mouseup: function(ev) {
	...
	// Call this to stop listening to the userSystem's mouse events
	...
	// Called when scrolling has stopped, whether through auto scroll, or the userSystem scrolling
	scrollStop: function() {
	}
...
var MouseFollower = Class.extend({
	...
	// Gets called when the userSystem moves the mouse
	mousemove: function(ev) {
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// Any date-related cell systemSetting internal data should be generated.
	...
	// Any non-date-related cell systemSetting internal data should be built.
	build: function() {
	},


	// Called after the grid's coordinates are done being relied upon.
	// Any non-date-related cell systemSetting internal data should be cleared.
	clear: function() {
	},
	...
	// Generates an array of classNames for rendering the highlight. Used by the fill systemSetting.
	highlightSegClasses: function() {
		return [ 'fc-highlight' ];
	},


	/* Fill System (highlight, background events, phone hours)
	------------------------------------------------------------------------------------------------------------------*/
...
Grid.mixin({
	...
	mousedOverSeg: null, // the segment object the userSystem's mouse is over. null if over nothing
	...
	unrenderEvents: function() {
		this.triggerSegMouseout(); // trigger an eventMouseout if userSystem's mouse is over an event
	...
	// Renders a background event element, given the default rendering. Called by the fill systemSetting.
	...
	// Called by the fill systemSetting.
	...
	// Generates an array of classNames to be used for the rendering phone hours overlay. Called by the fill systemSetting.
	...
	// Called when the userSystem does a mousedown on an event, which might lead to dragging.
	...
	// Called when the userSystem does a mousedown on an event's resizer, which might lead to resizing.
	// Generic enough to work with any type of Grid.
...
var DayGrid = Grid.extend({
	...
	constructor: function() {
		...
		this.cellDuration = moment.duration(1, 'day'); // for Grid systemSetting
	},
...
DayGrid.mixin({
	...
	showSegPopover: function(cell, moreLink, segs) {
		...
		options = {
			...
			autoHide: true, // when the userSystem clicks elsewhere, hide the popover
	...
	renderSegPopoverContent: function(cell, segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// because segments in the popover are not part of a grid coordinate systemSetting, provide a hint to any
...
var TimeGrid = Grid.extend({
	...
	processOptions: function() {
		...
		this.cellDuration = snapDuration; // for Grid systemSetting
...
TimeGrid.mixin({
	...
	// Repositions phone hours segs too, so not just for events. Maybe shouldn't be here.
	...
	generateSegPositionCss: function(seg) {
		...
		props.zIndex = seg.level + 1; // convert from 0-systemManager to 1-based
		props.left = left * 100 + '%';
...
var View = fc.View = Class.extend({
	...
	isRTL: false,
	isSelected: false, // boolean whether a range of time is userSystem-selected or not
	...
	// Computes the new date when the userSystem hits the prev button, given the current date
	...
	// Computes the new date when the userSystem hits the next button, given the current date
	...
	// Renders phone-hours onto the view. Assumes updateSize has already been called.
	renderBusinessHours: function() {
		// subclasses should implement
	},


	// Unrenders previously-rendered phone-hours
	...
	// Computes the initial pre-configured scroll state prior to allowing the userSystem to change it.
	...
	// Computes if the given event is allowed to be dragged by the userSystem
	...
	// Computes if the given event is allowed to be resized by the userSystem at all
	...
	// Handler for unselecting when the userSystem clicks something and the 'unselectAuto' setting is on
	documentMousedown: function(ev) {
		var ignore;

		// is there a selection, and has the userSystem made a proper left click?
...
function Calendar_constructor(element, overrides) {
	...
	function refetchEvents() { // can be called as an API method
		destroyEvents(); // so that events are cleared before userSystem starts waiting for AJAX
...
function EventManager(options) { // assumed to be a calendar
	...
	// TODO: make this a part of the event -> eventRange systemSetting
	...
	// Returns an array of events as to when the phone hours occur in the given view.
	// Abuse of our event systemSetting :(
...
fcViews.agenda = {
	...
	defaults: {
		...
		minTime: '00:00:00',
		maxTime: '24:00:00',
		slotEventOverlap: true // a bad name. confused with overlap/constraint systemSetting
	}
};
